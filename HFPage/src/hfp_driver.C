...
{
    Status answer;
    minibase_globals = new SystemDefs(answer, dbpath, logpath, 100, 500, 100, "Clock");
    if (answer == OK)
...
int HfpDriver::test1()
{
    ...
    hfp.init(tmpPID);
    tmpPID = 8;
    hfp.setNextPage(tmpPID);
    tmpPID = INVALID_PAGE;
    hfp.setPrevPage(tmpPID);

    cout << "Current Page No.: " << hfp.page_no() << ", " << "Next Page No.: " << hfp.getNextPage() << ", "
         << "Prev Page No.: " << hfp.getPrevPage() << ", " << "Available Space: " << hfp.available_space() << endl;
    if (hfp.empty() != true)
    {
...
int HfpDriver::test2()
{
    ...
    for (i = 0; i < Limit; i++)
    {
        ...
        if (status != OK)
        {
            ...
            cout << "    Current Page No.: " << hfp.page_no() << ", " << "Available Space: " << hfp.available_space()
                 << endl;
        } else
        {
            cout << "Inserted record, RID " << Rid.pageNo << ", " << Rid.slotNo << endl;
        }
    }
    ...
    while (status == OK)
    {
        ...
        {
            cout << "Retrieved record, RID " << Rid.pageNo << ", " << Rid.slotNo << endl;
        }
        if (size != sizeof(int))
...
int HfpDriver::test3()
{
    ...
    for (i = 0; i < deleteListSize; i++)
    {
        ...
        if (status != OK)
        {
            cout << "ERROR: Deletion of Rid(" << Rid.pageNo << "," << Rid.slotNo << ") failed\n";
        }
    }
    ...
    while (status == OK)
    {
        cout << "Retrieving record " << Rid.pageNo << ", " << Rid.slotNo << endl;
        status = hfp.getRecord(Rid, TmpBuf, size);
        if (status != OK)
        {
            cout << "ERROR: getting record.\n";
...
int HfpDriver::test4()
{
    ...
    for (i = 0; i < Limit; i++)
    {
        ...
        if (i % 2 == 0)
        {
            strcat(TmpBuf1, Letter);
            size = strlen(TmpBuf1);
            status = hfp.insertRecord(TmpBuf1, size + 1, Rid);
        } else
        {
            strcat(TmpBuf2, Letter);
            size = strlen(TmpBuf2);
...
int HfpDriver::test5()
{
    ...
    hfp.init(tmpPID);
    for (i = 0; i < 2000; i++) LongStr[i] = (char) (i % 26 + 65);
    LongStr[2000] = '\0';
...
int HfpDriver::test6()
{
    ...
    while (1)
    {
        ...
        *dp = records[nrRecords].value;
        status = hfp.insertRecord(garbage, records[nrRecords].length, records[nrRecords].rid);
        if (status == OK)
        {
    ...
    for (int i = 0; i < nrRecords; i++)
    {
        ...
        } else
        {
            ...
            {
                cout << "ERROR: record " << curRec << " has length " << ret_len << " should be "
                     << records[curRec].length << endl;
            }

            if (*whered != records[curRec].value)
            {
                cout << "ERROR: record " << curRec << "has value" << *whered << "should have value "
                     << records[curRec].value << endl;
            }
        }
        ...
        if (status != OK)
        {
            cout << "ERROR: failed to delete record " << curRec << endl;
        }
    ...
    {
        cout << "ERROR: initial space does not match final space" << endl;
    }
